...
class MyNotes {
  constructor() {
    if (document.querySelector("#my-notes")) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults.headers.common["X-WP-Nonce"]) = FictionalUniversityData.nonce;
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(FictionalUniversityData.rootUrl + "/wp-json/wp/v2/note/", ourNewPost);
      if (response.data != "Note Limit Exceed") {
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(FictionalUniversityData.rootUrl + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log("Sorry");
    }
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](FictionalUniversityData.rootUrl + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
      ...
      if (response.data.count_users_notes < 5) {
...
class Search {
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(`${FictionalUniversityData.rootUrl}/wp-json/university/v1/search?term=${this.searchInput.val()}`, results => {
      //language=HTML
